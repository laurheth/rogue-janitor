...
Player.prototype.clean = function(verb) {
    ...
    if (doSelf != null) {
        if (doSelf.unique) {
            Game.sendMessage("Stand next to "+doSelf.name+" to "+verb+" it.");
        }
        else {
            let message="Stand next to the "+doSelf.name.toLowerCase()+" to "+verb;
            if (doSelf.name.charAt(doSelf.name.length-1)=='s') {
                message+=" them.";
            }
            else {
                message+=" it.";
